...
knitr::opts_chunk$set(echo = TRUE, cache = FALSE, fig.width = 7, fig.height = 5, out.width = "67%", out.height = "67%", optipng = '-o7', rownames.print = FALSE, rows.print = 10, cols.min.print = 8)
options(width = 240)
```
...
## Coding performance measures

```{r coding_perf}
data <- data %>% dplyr::arrange(country, year) %>% 
  mutate(period = cut(year, seq(1960, 2020, 5), right = TRUE))
data <- data %>% group_by(country, period) %>% 
  mutate(pc_rgdpe_avg = mean(pc_rgdpe_pwt, na.rm = TRUE))
data$pc_rgdpe_avg[is.nan(data$pc_rgdpe_avg)] <- NA
...
take_logs <- c("pc_rgdpe_avg", "life_exp_wpp", "mys_ratio_hdr")
...
## summary(data[, c(take_logs_negative, paste("lg", take_logs_negative, sep = "_"))]) 
...
names(vars) <- vars <- c(variables$health, variables$gender)
names(years) <- years <- c(seq(1965, 2015, 5), 2018)
performance_measures <- lapply(vars, function(var) {
  results <- lapply(years, function(year) {
    dat <- data[data$year == year, c("country", "year", "lg_pc_rgdpe_avg", var)]
      if(nrow(na.omit(dat)) > 0) {
        CodePerformance(y_var = var, x_vars = "lg_pc_rgdpe_avg", df = dat)
      } 
  })
...
### Merging the classifications into the dataset for analyses
...
```{r trajectories, echo = FALSE, eval = FALSE, message = FALSE, warning = FALSE, results = 'hide'}
lapply(c(variables$health_full, variables$gender_full), function(var) {
  dat <- df[df$class1995 %in% c("H1G1", "H1G2", "H2G1", "H2G2", "H4G4", "H4G5", "H5G4", "H5G5"), ]
  ggplot(dat, aes(x = year, y = !!sym(var), group = class1995, color = class1995)) + 
...
## figures
pdf(file = paste("pdfs/trend_life_exp_wpp.pdf", sep = ""), width = 16, height = 9)
dat <- na.omit(df_new[, c("year", "avg_life_exp_wpp", "low_life_exp_wpp", "upp_life_exp_wpp")])
matplot(x = dat$year, y = dat[, -1], type = "l", xlab = "", ylab = "", 
        main = "Life Expectancy", lwd = 2, lty = "solid", col = c("black", "red", "blue"))
legend("bottomright", legend = c("Global Average", "low classification", "high classification"), 
       lty = "solid", lwd = 2, col = c("black", "red", "blue"))
dev.off()
pdf(file = paste("pdfs/trend_imr_wpp.pdf", sep = ""), width = 16, height = 9)
dat <- na.omit(df_new[, c("year", "avg_imr_wpp", "low_imr_wpp", "upp_imr_wpp")])
matplot(x = dat$year, y = dat[, -1], type = "l", xlab = "", ylab = "", 
        main = "Infant Mortality Rate", lwd = 2, lty = "solid", col = c("black", "red", "blue"))
legend("topright", legend = c("Global Average", "low classification", "high classification"), 
       lty = "solid", lwd = 2, col = c("black", "red", "blue"))
dev.off()
pdf(file = paste("pdfs/trend_mys_ratio_hdr.pdf", sep = ""), width = 16, height = 9)
dat <- na.omit(df_new[, c("year", "avg_mys_ratio_hdr", "low_mys_ratio_hdr", "upp_mys_ratio_hdr")])
matplot(x = dat$year, y = dat[, -1], type = "l", xlab = "", ylab = "", 
        main = "Mean Years of Schooling", lwd = 2, lty = "solid", col = c("black", "red", "blue"))
legend("bottomright", legend = c("Global Average", "low classification", "high classification"), 
       lty = "solid", lwd = 2, col = c("black", "red", "blue"))
dev.off()
pdf(file = paste("pdfs/trend_asfr_adol_wpp.pdf", sep = ""), width = 16, height = 9)
dat <- na.omit(df_new[, c("year", "avg_asfr_adol_wpp", "low_asfr_adol_wpp", "upp_asfr_adol_wpp")])
matplot(x = dat$year, y = dat[, -1], type = "l", xlab = "", ylab = "", 
        main = "Adolescent Fertility Rate", lwd = 2, lty = "solid", col = c("black", "red", "blue"))
legend("topright", legend = c("Global Average", "low classification", "high classification"), 
       lty = "solid", lwd = 2, col = c("black", "red", "blue"))
dev.off()
```

### Trends by classification group
...
### Cumulative death rate trends by 1995 classification groups
...
### Cumulative repression trends by 1995 classification groups
...
## Preparing cross-sectional dataset for analyses
```{r reg_prep}
...
names(new1990)[names(new1990) != "country"] <- paste(names(new1990)[names(new1990) != "country"], 
                                                     "1990", sep = "_")
names(new1995)[names(new1995) != "country"] <- paste(names(new1995)[names(new1995) != "country"], 
                                                     "1995", sep = "_")
names(new2015)[names(new2015) != "country"] <- paste(names(new2015)[names(new2015) != "country"], 
                                                     "2015", sep = "_")
...
# summary(new[, paste(covariates, "_growth1995_2015", sep = "")])
...
Performance <- function(category, year, vars) {
  ...
  lapply(vars, function(var) {
    ...
    # other <- c("deaths_all_ucdp_rate_cumulative")
    # other <- paste(other, year + 1, "_2015", sep = "")
    other <- NULL
