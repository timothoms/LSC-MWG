...
RunModels <- function(equations,
  ...
  test_args_list <- list(
    ## unit effects
    list(testfunction = "pFtest", x_estimator = "fe", z_estimator = "ols", what = "unit effects"),
    list(testfunction = "plmtest", x_estimator = "ols", what = "unit effects"),
    list(testfunction = "plmtest", x_estimator = "ols", effect = "time", what = "unit effects"),
    list(testfunction = "plmtest", x_estimator = "ols", effect = "twoways", what = "unit effects"),
    ## correlated effects
    list(testfunction = "phtest", x_estimator = "fe", x2_estimator = "re", what = "correlated effects"),
    list(testfunction = "phtest", x_estimator = "fegls", x2_estimator = "ggls", note = "(FGLS)", what = "correlated effects"),
    ## any type of serial corr. of time-decaying kind in the ideosyncratic error terms
    list(testfunction = "pwtest", x_estimator = "ols", what = "ideosyncratic serial corr."),
    ## serial corr.: detecting features of persistence
    list(testfunction = "pbsytest", x_estimator = "ols", test = "ar", what = "persistent serial corr."),
    list(testfunction = "pbsytest", x_estimator = "ols", test = "re", what = "persistent serial corr."),
    ## list(testfunction = "pbsytest", x_estimator = "ols", test = "j"),
    list(testfunction = "pbltest", x_estimator = "re", alternative = "onesided", what = "persistent serial corr."),
    ## traditional serial corr. tests
    ...
    list(testfunction = "pwartest", x_estimator = "fe", what = "traditional serial corr.")
  )
  ...
  results <- lapply(test_args_list, function(test_args) {
    tests <- lapply(models, function(modelname, args = test_args) {
      ...
      test_resultsmethod <- paste(test_results$testfunction, test_resultsmethod, sep = ": ")
      ...
      data.frame(test_results[c("method", "alternative", "p.value", "what")])
    })
    tests <- lapply(names(tests), function(modelname) {
      names(tests[[modelname]])[3] <- modelname
      return(tests[[modelname]])
    })
    Reduce(f = function(...) merge(..., by = c("method", "alternative", "what"), all = TRUE), x = tests)
  })
  tests <- do.call(rbind, results)
  tests[, !names(tests) %in% c("method", "alternative", "what")] <- 
...
```{r echo = FALSE, eval = params$incl_new, include = params$incl_new}
knitr::asis_output("##### NEW DV: DALYs (DALYs) {.tabset .tabset-fade .tabset-pills}")
```
```{r echo = FALSE, eval = params$incl_new, include = params$incl_new}
knitr::asis_output("###### Base models")
```
