...
source("_R/setup.R")
knitr::opts_chunk$set(cache = TRUE, fig.width = 12, fig.height = 10, 
...
summary(data[, c(variables$negative[1:6])])
summary(data[, c(variables$negative[7:9])])
data[, variables$negative] <- lapply(data[, variables$negative], '-')
summary(data[, c(variables$negative[1:6])])
summary(data[, c(variables$negative[7:9])])
```

## Coding performance measures for later use
