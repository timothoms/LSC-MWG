...
names(data)[str_detect(names(data), "gini")]


data %>% 
  filter(year >= 1970) %>%
  mutate(check = !is.na(gini_disp_mean)) %>% 
  select(year, check) %>% 
  table
table(data$year, !is.na(data$gini_disp_mean))
...
viol_vars_log <- c(viol_vars_log, 
                   paste(viol_vars_log, "rate", sep = "_"), 
