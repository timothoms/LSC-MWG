...
# summary(new)
new[new$country == "Afghanistan", ]

lapply(unique(new$country), function(ctry) {
  ggplot(new[new$country == ctry, ]) + 
    # geom_segment(arrow = arrow(angle = 15, type = "closed", length = unit(0.15, "inches"))) + 
    geom_segment(aes(x = lag_asfr_adol_wpp_avg, y = lag_imr_wpp_avg, xend = asfr_adol_wpp_avg, yend = imr_wpp_avg), 
                 lineend = "round") + 
    geom_point(aes(x = lag_asfr_adol_wpp_avg[period == min(period)],
                   y = lag_imr_wpp_avg[period == min(period)]), 
               shape = 22, size = 2.5, fill = "red") + 
    geom_point(aes(x = asfr_adol_wpp_avg[period == max(period)],
                   y = imr_wpp_avg[period == max(period)]), 
               shape = 23, size = 2.5, fill = "green") + 
    geom_abline(xintercept = mean(asfr_adol_wpp_avg)) + 
    geom_abline(yintercept = mean(imr_wpp_avg)) + 
    xlim(-330, -1) + ylim(-240, -1) + labs(x = "Gender", y = "Health") + theme_classic() + ggtitle(ctry)
})
```
