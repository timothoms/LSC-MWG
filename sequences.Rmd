...
# vars <- c("country", "year", "conf_intra", "wars_intra", "conflict_internal", "war_internal","latentmean_fariss", "v2clkill", "v2cltort", "v2caviol")
# table(df[, c("conf_intra", "wars_intra")])
ViolenceCats <- function(dataset = df[!(df$country %in% missing_countries), ], start_yr = 1961, end_yr = 1970, yrs_long = 3) {
...
violence <- list(
  ViolenceCats(start_yr = 1961, end_yr = 1970, yrs_long = 3), 
  ViolenceCats(start_yr = 1971, end_yr = 2015, yrs_long = 3),
  ViolenceCats(start_yr = 1981, end_yr = 1990, yrs_long = 3), 
  ViolenceCats(start_yr = 1991, end_yr = 2015, yrs_long = 3) 
)
...
lapply(unique(df$country), function(ctry) {
  ggplot(data = df[df$country == ctry & !is.na(df$lag_health) & !is.na(df$lag_gender), ], 
    ...
    scale_color_manual(values = c(black = "black", gray = "gray", 
                                  "1970" = "gray", "1975" = "gray", "1980" = "red", "1985" = "orange", "1990" = "red", 
                                  "1995" = "orange", "2000" = "purple", "2005" = "blue", "2010" = "purple", "2015" = "blue")) +
    ...
    ggtitle(label = ctry, subtitle = paste("\nfrom 1970: ", seq$typology1970[seq$country == ctry],
...
#### By 1995 classification (from origin)
...
#### Typology from 1970
```{r typology1970, results = 'hide', fig.width = 5, fig.height = 5, out.width = "33%", out.height = "33%", fig.show = "hold", warning = FALSE}
...
lookup <- table(seq$typology1970)
lapply(sort(unique(df$typology1970)), function(sequ) {
  plotCategory(cat = sequ, 
           dat = df[df$typology1970 == sequ & df$period > 1970, ], 
           range = range_both)
})
```
...
#### Summary data
```{r table, rows.print = 12, cols.print = 10, cols.min.print = 10}
new[, c("country", "typology1970", "typology1975", "typology1995", "class_vv1975", "class_vv1995")]
new[, c("country", "first", "periods", "total", "distance", "dist_total", "dist_period", "health_period", "gender_period")]
summary(new[, c("first", "periods", "total", "distance", "dist_total", "dist_period", "health_period", "gender_period")])
```

#### Health & gender change
```{r bivariate, fig.show = "hold", fig.width = 5, fig.height = 5, out.width = "33%", out.height = "33%", message = FALSE}
...
#### Internal conflict before
```{r conflict_before, results = 'hide', fig.show = "hold", fig.width = 5, fig.height = 5, out.width = "33%", out.height = "33%", warning = FALSE}
lookup <- table(new$conflict_1961_1970)
lapply(c("none", "conflict", "conflict (3+ yrs)", "war", "war (3+ yrs)"), function(cat) {
  plotCategory(cat = cat, 
           dat = df[df$conflict_1961_1970 == cat & df$period > 1975, ], 
           range = range_both)
})
```
...
round(addmargins(prop.table(table(new[, c("typology1975", "conflict_1971_2015")])[, c(3, 1:2, 4:5)], margin = 2) * 100, margin = 1), 1)
new[, c("country", "conflict_1961_1970", "conflict_1971_2015", "conflict_1981_1990", "conflict_1991_2015")]
```

#### LPI
```{r lpi, results = 'hide', fig.show = "hold", fig.width = 5, fig.height = 5, out.width = "33%", out.height = "33%", warning = FALSE}
lookup <- table(new$latentmean_fariss_1971_2015)
lapply(c("1st quartile", "2nd quartile", "3rd quartile", "4th quartile"), function(cat) {
  plotCategory(cat = cat, 
           dat = df[df$latentmean_fariss_1971_2015 == cat & df$period > 1975, ], 
           range = range_both)
})
```
```{r lpi_data, rows.print = 12, cols.print = 10, cols.min.print = 10}
round(addmargins(prop.table(table(new[, c("typology1975", "latentmean_fariss_1971_2015")]), margin = 2) * 100, margin = 1), 1)
new[, c("country", "latentmean_fariss_1961_1970", "latentmean_fariss_1971_2015", "latentmean_fariss_1981_1990", "latentmean_fariss_1991_2015")]
```

#### Extra-judicial killings
```{r killings, results = 'hide', fig.show = "hold", fig.width = 5, fig.height = 5, out.width = "33%", out.height = "33%", warning = FALSE}
lookup <- table(new$v2clkill_1971_2015)
lapply(c("1st quartile", "2nd quartile", "3rd quartile", "4th quartile"), function(cat) {
  plotCategory(cat = cat, 
           dat = df[df$v2clkill_1971_2015 == cat & df$period > 1975, ], 
           range = range_both)
})
```
```{r killings_data, rows.print = 12, cols.print = 10, cols.min.print = 10}
round(addmargins(prop.table(table(new[, c("typology1975", "v2clkill_1971_2015")]), margin = 2) * 100, margin = 1), 1)
new[, c("country", "v2clkill_1961_1970", "v2clkill_1971_2015", "v2clkill_1981_1990", "v2clkill_1991_2015")]
```

#### State torture
```{r torture, results = 'hide', fig.show = "hold", fig.width = 5, fig.height = 5, out.width = "33%", out.height = "33%", warning = FALSE}
lookup <- table(new$v2cltort_1971_2015)
lapply(c("1st quartile", "2nd quartile", "3rd quartile", "4th quartile"), function(cat) {
  plotCategory(cat = cat, 
           dat = df[df$v2cltort_1971_2015 == cat & df$period > 1975, ], 
           range = range_both)
})
```
```{r torture_data, rows.print = 12, cols.print = 10, cols.min.print = 10}
round(addmargins(prop.table(table(new[, c("typology1975", "v2cltort_1971_2015")]), margin = 2) * 100, margin = 1), 1)
new[, c("country", "v2cltort_1961_1970", "v2cltort_1971_2015", "v2cltort_1981_1990", "v2cltort_1991_2015")]
```

#### Societal violence
```{r societal, results = 'hide', fig.show = "hold", fig.width = 5, fig.height = 5, out.width = "33%", out.height = "33%", warning = FALSE}
lookup <- table(new$v2caviol_1971_2015)
lapply(c("1st quartile", "2nd quartile", "3rd quartile", "4th quartile"), function(cat) {
  plotCategory(cat = cat, 
           dat = df[df$v2caviol_1971_2015 == cat & df$period > 1975, ], 
           range = range_both)
})
```
```{r societal_data, rows.print = 12, cols.print = 10, cols.min.print = 10}
round(addmargins(prop.table(table(new[, c("typology1975", "v2caviol_1971_2015")]), margin = 2) * 100, margin = 1), 1)
new[, c("country", "v2caviol_1961_1970", "v2caviol_1971_2015", "v2caviol_1981_1990", "v2caviol_1991_2015")]
```
