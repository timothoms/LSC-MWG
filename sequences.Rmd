...
missing_countries <- c("Montenegro", "Serbia", "Timor-Leste", "Andorra", "Czechoslovakia", "Dominica", "East Germany", "Kosovo", "Liechtenstein", "Marshall Islands", "Monaco", "Nauru", "North Vietnam", "North Yemen", "Palau", "San Marino", "Saint Kitts & Nevis", "South Vietnam", "South Yemen", "Soviet Union", "Tuvalu", "West Germany", "Yugoslavia SFR", "South Sudan", "Serbia & Montenegro")
# table(df[, c("conf_intra", "wars_intra")])
df <- df[!(df$country %in% missing_countries), ]
df <- df[!(df$country == "Ethiopia" & df$year < 1991), ]
seq <- seq[!(seq$country %in% missing_countries), ]
ViolenceVars <- function(dataset = df, start_yr = 1975, end_yr = 2015, yrs_long = 3) {
  df <- dataset %>% 
    select(country, year, 
...
df <- df %>% 
  ...
  group_by(country) %>% 
  mutate(lag_imr_wpp_avg = dplyr::lag(imr_wpp_avg), 
         lag_asfr_adol_wpp_avg = dplyr::lag(asfr_adol_wpp_avg),
         lag_life_exp_wpp_avg = dplyr::lag(life_exp_wpp_avg),
         lag_mys_age_ratio_ihme_avg = dplyr::lag(mys_age_ratio_ihme_avg),
         lag_health = dplyr::lag(health),
         lag_gender = dplyr::lag(gender))
...
new <- merge(seq[, names(seq)[!str_detect(names(seq), "sequence")]], new, by = "country", all.y = TRUE)
new <- merge(new, violence, by = "country", all = TRUE)
df <- merge(df, seq[, names(seq)[!str_detect(names(seq), "sequence")]], by = "country", all.x = TRUE)
...
lapply(unique(df$country), function(ctry) { # ctry <- "Ethiopia"
